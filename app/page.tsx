'use client'

import React from 'react'
import { Header } from '@/components/navigation/header'
import SentinelHero from '@/components/sentinel-hero'
import { ContractInspector } from '@/components/contract-inspector'
import { HowItWorks } from '@/components/how-it-works'
import { OracleFeed } from '@/components/oracle-feed'
import { DeveloperTab } from '@/components/developer-tab'
import { ShieldCheck, ExternalLink, Lock } from 'lucide-react'

export default function Page() {
  return (
    <div className="min-h-screen bg-slate-950 text-white selection:bg-cyan-500/30 font-sans">
      {/* Top sticky navbar */}
      <Header />

      {/* Main Content */}
      <main>
        {/* WebGL Animated Aurora Hero with SentinelZK Context */}
        <SentinelHero />

        {/* Contract Search & Attestation Verdict Tile */}
        <ContractInspector />

        {/* How SentinelZK Preserves Privacy (4-Step Circuit Explainer) */}
        <HowItWorks />

        {/* Live Oracle Attestation Feed & Statistics */}
        <OracleFeed />

        {/* Developer Integration Tab (TypeScript SDK & Compact Contract) */}
        <DeveloperTab />
      </main>

      {/* Footer */}
      <footer className="border-t border-white/10 bg-slate-950 py-12 px-4 sm:px-6 lg:px-8 text-xs text-slate-400">
        <div className="mx-auto max-w-7xl flex flex-col sm:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-3">
            <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-cyan-500/10 border border-cyan-500/20 text-cyan-400">
              <ShieldCheck className="h-4 w-4" />
            </div>
            <div>
              <div className="font-mono font-bold text-slate-200">
                Sentinel<span className="text-cyan-400">ZK</span>
              </div>
              <div className="text-[11px] text-slate-500">
                Built for Akindo WaveHack — Midnight Network Track
              </div>
            </div>
          </div>

          <div className="flex items-center gap-6 text-slate-400">
            <a
              href="https://midnight.network"
              target="_blank"
              rel="noopener noreferrer"
              className="hover:text-cyan-400 transition flex items-center gap-1"
            >
              <span>Midnight Network</span>
              <ExternalLink className="h-3 w-3" />
            </a>
            <a
              href="https://github.com"
              target="_blank"
              rel="noopener noreferrer"
              className="hover:text-cyan-400 transition flex items-center gap-1"
            >
              <svg className="h-3.5 w-3.5 fill-current" viewBox="0 0 24 24">
                <path fillRule="evenodd" clipRule="evenodd" d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.53 1.032 1.53 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z" />
              </svg>
              <span>GitHub (Apache 2.0)</span>
            </a>
          </div>

          <div className="text-right text-[11px] text-slate-500">
            Compact ZK Circuits &bull; On-Chain Safety Oracle &bull; Privacy-First DeFi
          </div>
        </div>
      </footer>
    </div>
  )
}
